...
class QuestaoController extends FOSRestController {
	...
	public function getQuestaosAction(Request $request, ParamFetcherInterface $paramFetcher) {
		...
		return $this->container->get ( 'question.questao.handler' )->all ( $limite, $posicao_inicio );
	}
	...
	public function postQuestaoAction(Request $request) {
		try {
			$newQuestao = $this->container->get ( 'question.questao.handler' )->post ( $request->request->all () );
			
			$routeOptions = array (
					'codigo' => $newQuestao->getCodigo (),
					'_format' => $request->get ( '_format' ) 
			);
			
			return $this->routeRedirectView ( 'aspe_get_questao', $routeOptions );
	...
	public function deleteQuestaoAction($codigo, Request $request, ParamFetcherInterface $paramFetcher) {
		try {
			if ($usuario = $this->container->get ( 'question.questao.handler' )->get ( $codigo )) {
				$statusCode = Codes::HTTP_CREATED;
				$this->container->get ( 'question.questao.handler' )->delete ( $usuario );
			} else
				$statusCode = Codes::HTTP_NO_CONTENT;
			$routeOptions = array (
					'_format' => $request->get ( '_format' ) 
			);
			return $this->routeRedirectView ( 'aspe_get_questaos', $routeOptions, $statusCode );
		} catch ( InvalidFormException $exception ) {
	...
	public function putQuestaoAction(Request $request, $codigo) {
		try {
			...
			$routeOptions = array (
					'codigo' => $questao->getCodigo (),
					'_format' => $request->get ( '_format' ) 
			);
			
			return $this->routeRedirectView ( 'aspe_get_questao', $routeOptions, $statusCode);
		} catch ( InvalidFormException $exception ) {
	...
	public function patchQuestaoAction(Request $request, $codigo) {
		try {			
			$questao = $this->container->get ( 'question.questao.handler' )->patch ( $this->getQuestaoAction ( $codigo ), $request->request->all () );
			...
			return $this->routeRedirectView ( 'aspe_get_questao', $routeOptions, Codes::HTTP_OK );
		} catch ( InvalidFormException $exception ) {
